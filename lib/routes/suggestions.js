...
router.get('/suggestions', function(request, response, next) {
    models.instance.suggestions.find({ /*$orderby: { "$desc": "created" } */ }, function(err, suggestions) {
        if (err) {
    ...
    if (!request.body) {
        ...
        suggestion.save(function(error, result) {
            if (error) {
                ...
                // response.json(suggestion.toJSON());
                models.instance.suggestions.findOne({ suggestion: suggestion.suggestion, posted_by: suggestion.posted_by }, { allow_filtering: true, limit: 1 }, function(err, newSuggestion) {
                    if (err) {
                        handleError(err, response, 400);
                    } else {
                        console.log(newSuggestion);
                        response.json(newSuggestion);
                    }
                });
            }
        });
